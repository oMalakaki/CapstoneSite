...
const addProuctFormAutomatic = document.getElementById(
  "add-product-form-automatic"
);
...
const addOrderButton = document.querySelector(".open-modal-button-order");
...
function resetSelect() {
  selectMenus = document.querySelectorAll("table select");
  
  for (let i = 0; i < selectMenus.length; i++) {
    if (selectMenus[i].value !== "default") {
      selectMenus[i].value = "default";
    }
  }
}
function modalOpen() {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (addOrderButton) {
    ...
    addOrderButton.onclick = function () {
      modal.style.display = "block";
      modalContent.innerHTML = addOrderForm.outerHTML;
    };
  }
